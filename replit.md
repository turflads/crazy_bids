...
- **Team Logo Support**: Added `logo` field to team configuration in `config.json` for customizable team branding (e.g., `/logos/mumbai.png`, `/logos/chennai.png`)
- **Grade Key Standardization**: Ensured grade keys in config (A, B, C) match the uppercase conversion in `playerLoader.ts` for consistent lookups

### Viewer Interface Enhancements
- **Removed Player List Tabs**: Simplified viewer page by removing the player list tabs that duplicated information from team standings
- **Clickable Team Cards**: Team cards in viewer dashboard now open dialogs showing purchased players with sold prices
- **Team Player Dialogs**: Clicking any team card displays a full-screen dialog with:
  - Team statistics (total players, purse used, purse remaining)
  - Grid of purchased players showing sold prices (via `PlayerCard` component)
  - Empty state when no players have been purchased

### Admin Panel Synchronization
- **Real-time Team Card Updates**: Implemented `teamStateVersion` state trigger in Admin.tsx to force re-render of team cards when:
  - A player is sold to a team
  - Auction is reset
